...
//  float pitch_new = -_PI_2 + atan2(q.w * q.w - q.x * q.x - q.y * q.y + q.z * q.z, 2 * (q.y * q.z + q.w * q.x)); // orientation initiale MPU6050 selon Y
  float pitch_new = _PI_2 - atan2(q.w * q.w + q.y * q.y - q.x * q.x + q.z * q.z, 2 * (q.x * q.z - q.w * q.y)); // nouvelle orientation MPU6050 selon x
...
int initIMU() {
  Wire.begin();
  Wire.setClock(400000); // 400kHz I2C clock. Comment this line if having compilation difficulties 400000
  // initialize device
